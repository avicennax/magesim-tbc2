...
static PyObject* __pyx_convert__to_py_struct__SimulationResult(struct SimulationResult s) {
  ...
  Py_DECREF(member);
  member = PyFloat_FromDouble(s.dps); if (unlikely(!member)) goto bad;
  if (unlikely(PyDict_SetItem(res, __pyx_n_s_dps, member) < 0)) goto bad;
  Py_DECREF(member);
  member = PyFloat_FromDouble(s.evocated_at); if (unlikely(!member)) goto bad;
  if (unlikely(PyDict_SetItem(res, __pyx_n_s_evocated_at, member) < 0)) goto bad;
  Py_DECREF(member);
  member = PyFloat_FromDouble(s.regened_at); if (unlikely(!member)) goto bad;
  if (unlikely(PyDict_SetItem(res, __pyx_n_s_regened_at, member) < 0)) goto bad;
  Py_DECREF(member);
  member = __pyx_convert_PyObject_string_to_py_std__in_string(s.log); if (unlikely(!member)) goto bad;
  if (unlikely(PyDict_SetItem(res, __pyx_n_s_log, member) < 0)) goto bad;
  Py_DECREF(member);
  return res;
  bad:
  Py_XDECREF(member);
